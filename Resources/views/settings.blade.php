...
<form class="form-horizontal margin-top margin-bottom" method="POST" action=""
    ...
    <div class="form-group{{ $errors->has('settings[aichatpanel.max_context_tokens]') ? ' has-error' : '' }}">
        ...
        <div class="col-sm-6">
            ...
            <p class="help-block">{{ __('Budget for the whole prompt: the conversation, the extra context, and the chat itself. A quarter of it is reserved for the conversation and the chat cannot take that share, however long it grows; the newest message is kept whatever else has to go. The panel says so whenever anything is left out.') }}</p>
        </div>
    </div>
